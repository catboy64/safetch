...
fn convert_memory(mem_str: String, unity: String) -> String {
    ...
    match unity.as_str() {
        "kib"   => mem_int = mem_int,
        "mib"   => mem_int = mem_int/1024,
        "gib"   => mem_int = mem_int/1048576,
        _       => mem_int = mem_int/1024
    }
    mem_int.to_string()
}
...
# memory unity [mib, gib, kib] (default=mib)
mem_unity=mib
";
...
fn main() {
    ...
    mem_total = convert_memory(mem_total, mem_unity.clone());
    mem_used = convert_memory(mem_used, mem_unity.clone());

    let custom_ascii_path = config_path.to_owned() + "safetch_custom.ascii";
    ...
    if mem_used != empty && mem_total != empty {
        let mem_line = mem_used + "/" + &mem_total + " " + &mem_unity;
